<script lang="ts">
  ...
  {visible ? "scale-100" : "scale-0"}" bind:this={logoCompany} >
          <img class="lg:w-[200px] w-[130px]" src={base+infoPortfolio.jobs[companyIndex].icon} alt=""/>
          <h1 class="text-[16px] lg:text-[18px] m-0 mt-6 text-center">{infoPortfolio.jobs[companyIndex].name}</h1>
          <h2 class="text-[12px] lg:text-[14px] text-center">{infoPortfolio.jobs[companyIndex].role}</h2>
          <p class="text-[12px] lg:text-[16px] font-medium mt-3 lg:mt-5 text-center np:text-14"> {infoPortfolio.jobs[companyIndex].about} </p>
  </div>
